...
#define OUTPUT_NAME "Register"
    ...
    if (strcmp(registered_account.password, input_buffer)) { /*confirm failed*/
        ...
        user_dbc = CreateUserDataBlock();
        UserWrite(&registered_account, user_dbc);
        switch (DataBlockAppend(USER_LIST_DIR, user_dbc->item,
                                user_dbc->item_number)) {
            case DATA_UNOPENED:
                DestructDataBlockContainer(user_dbc);
                ERROR_INFO("check user list status!");
                SWITCH(interface_StartPage);
            case DATA_PASSED:
                DestructDataBlockContainer(user_dbc);
...
void interface_UserDelete(pRuntimeData data) {
    static char input_buffer[maximum_password_length + 1];
#define OUTPUT_NAME "UserDelete"
    fpos_t begin, end;
    if (DataCheck(USER_LIST_DIR) == DATA_PASSED) {
        if (DataBlockLocate(USER_LIST_DIR, USER_ID_INDEX, data->user.username,
                            &begin, &end) != DATA_PASSED) {
            ERROR_INFO(
                "current user missed from user list OR user list broken!");
            SWITCH(interface_UserCenter);
        }
    } else {
        ERROR_INFO("user list file missed!");
        SWITCH(interface_UserCenter);
    }
    ...
    SET_MARK();
    system("cls");
    NORMAL_INFO("Warning: all the data of this account will be deleted.");
    NORMAL_INFO("Are your sure to delete it?(Y(yes)/!(cancel))");
    INPUT_PROMPT("option");
    INPUT(input_buffer, 1, PredefinedInputCheckYesOrCancel);
    /*password confirmation*/
    puts("Please input the password for verification.");
    INPUT_PROMPT("password");
    INPUT(input_buffer, maximum_password_length, PredefinedInputCheckNormal);
    /*verify password*/
    if (!strcmp(data->user.password, input_buffer)) {
        /*second confimation*/
        NORMAL_INFO(
            "Warning Again: all the data of this account will be deleted.");
        NORMAL_INFO("Are your sure to delete it?(Y(yes)/!(cancel))");
        INPUT_PROMPT("option");
        ATTEMP(GetInput(input_buffer, 1, PredefinedInputCheckYesOrCancel));
        /*continue deleting*/
        /*remove this account from user list*/
        switch (DataBlockDelete(USER_LIST_DIR, begin, end)) {
            case DATA_UNOPENED:
            case DATA_OCCUPIED:
                ERROR_INFO("delete failed, please check user list file!");
                SWITCH(interface_UserCenter);
            case DATA_PASSED:
                break;
        }
        NORMAL_INFO("user deleted.");
        Delay(1000);
        SWITCH(interface_StartPage);
    } else {
        ERROR_INFO("Password incorrected");
        GOTO_MARK();
    }
...
#define OUTPUT_NAME "ChangePassword"
    static char input_buffer[maximum_password_length + 1];
    pDataBlockContainer dbc;
    char ch;
    fpos_t begin, end;
    SET_MARK();
    system("cls");
    NORMAL_INFO("Please input the password for verification.");
    INPUT_PROMPT("password");
    INPUT(input_buffer, maximum_password_length, PredefinedInputCheckNormal);
    /*verify password*/
    if (!strcmp(data->user.password, input_buffer)) {
        SET_MARK();
        system("cls");
        NORMAL_INFO("password correct");
        NORMAL_INFO("please input new password")
        /*verify passed, change password*/
        INPUT_PROMPT("password");
        INPUT(input_buffer, maximum_password_length,
              PredefinedInputCheckNormal);
        strcpy(data->user.password, input_buffer);
        /*sync to file*/
        dbc = CreateUserDataBlock();
        UserWrite(&data->user, dbc);
        switch (DataBlockLocate(USER_LIST_DIR, USER_ID_INDEX,
                                data->user.username, &begin, &end)) {
            case DATA_UNOPENED:
            case DATA_BROKEN:
            case DATA_NON_EXIST:
                DestructDataBlockContainer(dbc);
                ERROR_INFO("check user list status!");
                SWITCH(interface_UserCenter);
            case DATA_PASSED:
                switch (DataBlockRewrite(USER_LIST_DIR, &begin, &end, dbc->item,
                                         dbc->item_number)) {
                    case DATA_UNOPENED:
                    case DATA_OCCUPIED:
                        DestructDataBlockContainer(dbc);
...
#define OUTPUT_NAME "ChangeType"
    ...
    SET_MARK();
    system("cls");
    NORMAL_INFO("type: manager(m) or receptionist(r)?");
    INPUT_PROMPT("type");
    INPUT(input_buffer, 1, NULL);
    switch (input_buffer[0]) {
        case 'r':
            data->user.type = USER_RECEPTIONIST;
            break;
        case 'm':
            sub_interface_ManagerAuthorization(data);
            data->user.type = USER_MANAGER;
            break;
        default:
            ERROR_INFO("input must be r or m!");
            GOTO_MARK();
    ...
    dbc = CreateUserDataBlock();
    UserWrite(&data->user, dbc);
    switch (DataBlockLocate(USER_LIST_DIR, USER_ID_INDEX, data->user.username,
                            &begin, &end)) {
        case DATA_UNOPENED:
        case DATA_BROKEN:
        case DATA_NON_EXIST:
...
#define OUTPUT_NAME "Logout"

    memset(&(data->user), 0, sizeof(data->user));
    NORMAL_INFO("LOGGING OUT...")
    Delay(1000);
    SWITCH(interface_StartPage);
